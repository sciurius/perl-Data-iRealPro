...
# If packaged, VERSION will return the subpackager version...
...
With NPP output, colour the glyphs in each cell according to the
colour codes specified.

The string consists of a series of colour specifications. You can use
the primary and secondary colours with single uppercase letters (R, G,
B, Y, M, C, K), or use lowercase colour names (red, green, etc.). A
number specifies a repeat factor, so 4RG is the same as RRRRGGGG.

The final two codes specify the colour of texts and special symbols.

The number of codes supplied must match the number of cells in the song.

An example for a 4x4 bar song in 4/4 time:

    8GBGorange4GM8GRG1KR
...
With PNG output, crops the size of the image.
