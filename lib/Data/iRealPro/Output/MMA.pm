...
# Last Modified On: Tue Jan 22 13:20:04 2019
# Update Count    : 552
...
sub to_mma {
    ...
    my ( $time, $time_d, $time_n ) = $song->timesig;
...
EOD
    my %sused = ( "" => 1 );
    $sused{$_->{type}}++ foreach @{$s->{content}};
    for ( "", qw( Intro Verse A B C D Coda ) ) {
	next unless $sused{$_};
	$mma .= "DefGroove " . style2mma( $style, $_ ) . "\n";
    }

    $mma .= <<EOD;
// End of style mappings.
...
EOD
	...
	$mma .= "Groove " . style2mma( $style, $type ) . "\n";
	foreach my $item ( @{ $section->{content} } ) {
		...
		foreach my $i ( @{ $item->{content} } ) {
			...
			elsif ( $j =~ /^slash repeat/ ) {
			    warn("Measure $mcnt should not start with /\n")
			      unless @chords;
			    push( @chords, "/" );
			}
			elsif ( $j =~ /^advance / ) {
		...
		$res .= " " . chords2mma( $time_d, $time_n, @chords );
		$res .= "   /* " . $item->{jump}->{text} ." */"
		  if $item->{jump};
		$mma .= $res . "\n";
	    }
...
sub _chords2mma {
    ...
    while ( @chords && $chords[0] eq "" ) {
	shift(@chords);
    }
    while ( @chords && $chords[-1] eq "" ) {
	pop(@chords);
    }

    # Number of actual chords.
    my $sc = scalar( grep { $_ ne "" } @chords );

    if ( $sc >= $d ) {	# all slots filled
	return "@chords";
    }
    return "@chords" if $sc == 1;    # Okay, MMA will fill.
...
sub interpret {
    ...
    # Cleanup and break into sections.
    my $res = $self->_interpret1($song);

    foreach ( @{ $res->{content} } ) {
	# Process repeats.
	$self->_interpret2($_);
	# Process jumps and measures.
	$self->_interpret3($_);
	$res->{hasjumps} ||= $_->{hasjumps};
    }
...
sub style2mma {
    my ( $style, $section ) = @_;
    $section = "" if $section eq "section";
    return $style . ucfirst($section);	# for now
}
...
sub Data::iRealPro::Song::timesig {
    my ( $self ) = @_;
    my $time = "4/4";
    foreach ( @{ $self->tokens } ) {
...
sub Data::iRealPro::Song::composer {
    my ( $self ) = @_;
    my @t = split( ' ', $self->{composer} );
    @t[0,1] = @t[1,0] if @t == 2;
    return "@t";
}
